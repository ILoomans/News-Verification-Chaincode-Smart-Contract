...
describe("MyPrivateAssetContract", () => {
    ...
    beforeEach(() => {
        ...
        ctx.stub.getPrivateData
            ...
            .resolves(Buffer.from('{"privateValue":"150"}'));
        const hashToVerify = crypto
            .createHash("sha256")
            .update('{"privateValue":"150"}')
            .digest("hex");
    ...
    describe("#myPrivateAssetExists", () => {
        it("should return true for a my private asset", async () => {
            await contract.myPrivateAssetExists(ctx, "001").should.eventually.be
                .true;
        });

        it("should return false for a my private asset that does not exist", async () => {
            await contract.myPrivateAssetExists(ctx, "003").should.eventually.be
                .false;
        });
    });
    ...
    describe("#createMyPrivateAsset", () => {
        it("should throw an error for a my private asset that already exists", async () => {
            await contract
                .createMyPrivateAsset(ctx, "001")
                .should.be.rejectedWith(
                    /The asset my private asset 001 already exists/
                );
        });
    ...
    describe("#readPrivateMyPrivateAsset", () => {
        it("should throw an error for a my private asset that does not exist", async () => {
            await contract
                .readMyPrivateAsset(ctx, "003")
                .should.be.rejectedWith(
                    /The asset my private asset 003 does not exist/
                );
        });

        it("should return a my private asset", async () => {
            await contract
                .readMyPrivateAsset(ctx, "001")
                .should.eventually.deep.equal({ privateValue: "150" });
    ...
    describe("#verifyMyPrivateAsset", () => {
        it("should return true if hash calculated from object provided matches the hash of the private data", async () => {
            ...
            ctx.stub.getPrivateDataHash
                .withArgs(collectionName, "001")
                .resolves(Buffer.from(hashToVerify, "hex"));
            const result: boolean = await contract.verifyMyPrivateAsset(
                ctx,
                mspid,
                "001",
                { privateValue: "125" }
        ...
        it("should return false if hash calculated from object provided does not match the hash of the private data", async () => {
            ctx.stub.getPrivateDataHash
                .withArgs(collectionName, "001")
                .resolves(Buffer.from("someHash"));
            const result: boolean = await contract.verifyMyPrivateAsset(
                ctx,
                mspid,
                "001",
                { privateValue: "someValue" }
            );
        ...
        it("should throw an error when user tries to verify an asset that doesnt exist", async () => {
            ...
            await contract
                .verifyMyPrivateAsset(ctx, mspid, "001", {
                    privateValue: "someValue",
                })
                .should.be.rejectedWith(
                    "No private data hash with the Key: 001"
                );
        });
    });
});
